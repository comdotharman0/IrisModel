...
from diabetes.functions import FunctionsForDiabetesModel
...
modelsselection.key = "mymodelselection"
with tables:
...
funcs = FunctionsForDiabetesModel(dir(sl),[],"sl",modelsselection,X_train,X_test,y_train,y_test,r2_score,mean_squared_error,mean_absolute_error,df,chartsselection,{
...
chartss= charts.selectbox('Select a Chart',
                          ...
                          key="my_charts_type",
                          on_change=funcs.SelectCharts,
                          #args=(df),
                          kwargs={"df":df,"chartsselection":chartsselection})
charts.header("Select Column for X-axis")
#x=[]
    #for i in df:
chartsx = charts.selectbox("Select a Column for X-axis",[k for k in df],key="ChartX",on_change=funcs.SelectCharts,kwargs={"df":df,"chartsselection":chartsselection})
        #x.append(chartsx)
charts.header("Select Column for Y-axis")
    #y=[]
    #for i in df:
chartsy = charts.selectbox("Select a Column for Y-axis",[k for k in df],key="ChartY",on_change=funcs.SelectCharts,kwargs={"df":df,"chartsselection":chartsselection})
        #x.append(chartsy)
...
models = modelsselection.selectbox("Select a Model",funcs.modelss,key="my_models",
                          on_change=funcs.SelectModels, 
           #args=(modelsselection)
                         # kwargs={"_tabname": "mymodelsselection"}
                         ) 
    
modelsselection.header("Model Selected: "+str(models))
modelmetrics = modelsselection.expander("Model Metrics")
predsss=modelsselection.expander("Make a Prediction")
values = []
...
data["ModelName"]=data["ModelName"].astype('string')
